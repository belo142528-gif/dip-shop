...
def admin_login():
    ...
    if request.method == 'POST':
        if request.form.get('password') == ADMIN_PASSWORD:
            session['admin'] = True
            return redirect('/admin/dashboard')
    return '''<!DOCTYPE html><html><head><meta charset="utf-8"><title>Вход</title><style>body{font-family:system-ui;background:#f0f4f8;display:flex;justify-content:center;align-items:center;height:100vh}.login{background:#fff;padding:40px;border-radius:15px;width:300px;text-align:center}input{width:100%;padding:10px;margin:10px 0;border:1px solid #d1d5db;border-radius:8px}button{width:100%;padding:10px;background:#2563eb;color:#fff;border:none;border-radius:8px;cursor:pointer}</style></head><body><div class="login"><h2>Вход</h2><form method="POST"><input type="password" name="password" placeholder="Пароль" required><button type="submit">Войти</button></form></div></body></html>'''
...
def admin_dashboard():
    ...
    html = '''<!DOCTYPE html><html><head><meta charset="utf-8"><title>Админка</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui;background:#f0f4f8;padding:20px}.container{max-width:1200px;margin:0 auto;background:#fff;border-radius:15px;padding:25px}h1{color:#2563eb;margin-bottom:15px}.stats{display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:15px;margin:15px 0}.stat-card{background:#f8fafc;padding:15px;border-radius:10px;text-align:center}.stat-card .number{font-size:24px;font-weight:700;color:#2563eb}.stat-card .label{font-size:12px;color:#64748b}table{width:100%;border-collapse:collapse;margin-top:15px}th,td{padding:10px;text-align:left;border-bottom:1px solid #e5e7eb}th{background:#f8fafc;font-weight:600}.btn{display:inline-block;padding:8px 16px;border:none;border-radius:8px;cursor:pointer;text-decoration:none;font-size:14px}.btn-primary{background:#2563eb;color:#fff}.btn-danger{background:#dc2626;color:#fff}.btn-success{background:#16a34a;color:#fff}.btn-warning{background:#f59e0b;color:#fff}.btn-sm{padding:4px 10px;font-size:12px}.form-group{margin-bottom:12px}.form-group label{display:block;font-weight:600;margin-bottom:4px;font-size:14px}.form-group input,.form-group select{width:100%;padding:8px 12px;border:1px solid #d1d5db;border-radius:8px}.form-row{display:grid;grid-template-columns:1fr 1fr;gap:15px}.pagination{display:flex;gap:5px;margin:15px 0;flex-wrap:wrap}.pagination a,.pagination span{padding:8px 14px;border:1px solid #e5e7eb;border-radius:8px;text-decoration:none;color:#2563eb}.pagination .active{background:#2563eb;color:#fff;border-color:#2563eb}.header-actions{display:flex;gap:10px;flex-wrap:wrap;align-items:center}@media(max-width:600px){.form-row{grid-template-columns:1fr}}</style></head><body><div class="container"><div class="header-actions"><h1>Админка</h1><a href="/" class="btn btn-primary">В чат</a><a href="/admin/logout" class="btn btn-danger">Выйти</a></div><div class="stats"><div class="stat-card"><div class="number">''' + str(total) + '''</div><div class="label">Всего</div></div><div class="stat-card"><div class="number">''' + str(in_stock) + '''</div><div class="label">В наличии</div></div><div class="stat-card"><div class="number">''' + str(len(categories)) + '''</div><div class="label">Категорий</div></div></div><h2>Добавить товар</h2><form method="POST" action="/admin/add"><div class="form-row"><div class="form-group"><label>Название</label><input type="text" name="name" required></div><div class="form-group"><label>Цена</label><input type="number" name="price" required></div></div><div class="form-row"><div class="form-group"><label>Наличие</label><select name="stock"><option value="да">В наличии</option><option value="под заказ">Под заказ</option><option value="нет">Нет</option></select></div><div class="form-group"><label>Категория</label><input type="text" name="category"></div></div><div class="form-row"><div class="form-group"><label>Бренд</label><input type="text" name="brand"></div><div class="form-group"><label>Характеристики</label><input type="text" name="specs"></div></div><div class="form-group"><label>Описание</label><input type="text" name="description"></div><button type="submit" class="btn btn-success">Добавить</button></form><h2 style="margin-top:20px;">Товары (''' + str(total) + ''')</h2><div class="pagination">'''
    for p in range(1, total_pages + 1):
        if p == page:
            html += '<span class="active">' + str(p) + '</span>'
        else:
            html += '<a href="?page=' + str(p) + '">' + str(p) + '</a>'
    html += '''</div><table><tr><th>Название</th><th>Цена</th><th>Наличие</th><th>Категория</th><th>Действия</th></tr>'''
    for p in products:
        html += '<tr><td><strong>' + str(p[1]) + '</strong><br><small>' + str(p[7] or '') + ' ' + str(p[5] or '') + '</small></td><td>' + str(p[2]) + ' руб</td><td>' + str(p[3]) + '</td><td>' + str(p[4] or '-') + '</td><td><a href="/admin/edit/' + str(p[0]) + '" class="btn btn-warning btn-sm">Edit</a> <a href="/admin/delete/' + str(p[0]) + '" class="btn btn-danger btn-sm" onclick="return confirm(\'Удалить?\')">Del</a></td></tr>'
...
def admin_edit(product_id):
    ...
    if request.method == 'POST':
        ...
        return redirect('/admin/dashboard')
    s = 'да' if product[3] == 'да' else ''
    po = 'под заказ' if product[3] == 'под заказ' else ''
    n = 'нет' if product[3] == 'нет' else ''
    html = '''<!DOCTYPE html><html><head><meta charset="utf-8"><title>Редактировать</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui;background:#f0f4f8;padding:20px}.container{max-width:600px;margin:0 auto;background:#fff;border-radius:15px;padding:25px}h1{color:#2563eb}.form-group{margin-bottom:12px}.form-group label{display:block;font-weight:600;margin-bottom:4px}.form-group input,.form-group select{width:100%;padding:8px 12px;border:1px solid #d1d5db;border-radius:8px}.actions{margin-top:15px;display:flex;gap:10px}.btn{display:inline-block;padding:8px 16px;border:none;border-radius:8px;cursor:pointer;text-decoration:none}.btn-success{background:#16a34a;color:#fff}.btn-secondary{background:#6b7280;color:#fff}</style></head><body><div class="container"><h1>Редактировать</h1><form method="POST"><div class="form-group"><label>Название</label><input type="text" name="name" value="''' + str(product[1]) + '''" required></div><div class="form-group"><label>Цена</label><input type="number" name="price" value="''' + str(product[2]) + '''" required></div><div class="form-group"><label>Наличие</label><select name="stock"><option value="да" ''' + ('selected' if product[3]=='да' else '') + '''>В наличии</option><option value="под заказ" ''' + ('selected' if product[3]=='под заказ' else '') + '''>Под заказ</option><option value="нет" ''' + ('selected' if product[3]=='нет' else '') + '''>Нет</option></select></div><div class="form-group"><label>Категория</label><input type="text" name="category" value="''' + str(product[4] or '') + '''"></div><div class="form-group"><label>Бренд</label><input type="text" name="brand" value="''' + str(product[7] or '') + '''"></div><div class="form-group"><label>Характеристики</label><input type="text" name="specs" value="''' + str(product[5] or '') + '''"></div><div class="form-group"><label>Описание</label><input type="text" name="description" value="''' + str(product[6] or '') + '''"></div><div class="actions"><button type="submit" class="btn btn-success">Сохранить</button><a href="/admin" class="btn btn-secondary">Отмена</a></div></form></div></body></html>'''
    return html
...
if __name__ == '__main__':
    ...
    print(f"Elli running on port {port}")
